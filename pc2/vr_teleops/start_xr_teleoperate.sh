...
main() {
  ...
  teleop_log "Starting xr_teleoperate with DDS interface ${G1_TELEOP_DDS_IFACE}, img-server-ip ${G1_TELEOP_IMG_SERVER_IP}, input mode ${G1_TELEOP_INPUT_MODE}, ee ${G1_TELEOP_EE}"
  ...
  exec python teleop_hand_and_arm.py \
    --input-mode "${G1_TELEOP_INPUT_MODE}" \
    --arm "${G1_TELEOP_ARM}" \
    --ee "${G1_TELEOP_EE}" \
